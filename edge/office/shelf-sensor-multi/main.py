"""
Multi-item shelf sensor node — HX711 load cell + GM65 barcode scanner.

Publishes weight readings and barcode scans to MQTT.
Topic: office/{zone}/sensor/{device_id}/weight  {"value": 342.5}
       office/{zone}/sensor/{device_id}/barcode {"value": "4901234567890"}
...
def main():
    ...
    # Initialize sensors via SensorRegistry
    from sensor_registry import SensorRegistry
    from board_pins import get_board_pins

    pins = get_board_pins("esp32")
        ...
        try:
            ...
            if now - last_report > device.report_interval:
                try:
                    ...
                    if data:
                        # Publish weight and barcode on separate channels
                        if "weight" in data:
                            device.publish_sensor_data({"weight": data["weight"]})
                        if "barcode" in data:
                            device.publish_sensor_data({"barcode": data["barcode"]})
                except OSError:
                    print("Failed to read sensor")
                last_report = now

            # Check barcode more frequently (every 100ms)
            if _gm65:
                barcode_data = _gm65.read_barcode(timeout_ms=50)
                if barcode_data:
                    device.publish_sensor_data({"barcode": barcode_data})
