...
def get_device_config(fleet: dict, mac: str, dev_type: str) -> dict:
    ...
    # Assign static IP from pool if available
    pool = fleet.get("static_ip_pool", [])
    used_ips = {d.get("static_ip") for d in devices.values() if d.get("static_ip")}
    for ip in pool:
    ...
    save_fleet(fleet)
    log.info("Auto-assigned %s -> %s (zone=%s, ip=%s)", mac_upper, device_id, zone, cfg.get("static_ip", "DHCP"))
    return cfg
...
def generate_config_h(fleet: dict, device_cfg: dict, fw_dir: Path) -> Path:
    ...
    static_ip = device_cfg.get("static_ip", "")
    gateway = device_cfg.get("gateway", defaults.get("gateway", "192.168.128.1"))
    subnet = device_cfg.get("subnet", defaults.get("subnet", "255.255.255.0"))
    ...
    lines = [
        ...
        "",
        f'#define CFG_STATIC_IP    "{static_ip}"',
        f'#define CFG_GATEWAY      "{gateway}"',
        f'#define CFG_SUBNET       "{subnet}"',
        "",
    ]
...
def flash_device(port: str, fleet: dict, mqtt_verify: bool = False) -> bool:
    ...
    with build_lock:
        ...
        # 6. Build (keep generated_config.h until upload completes)
    ...
    # Clean up generated config after successful upload
    cleanup_config_h(fw_dir)
