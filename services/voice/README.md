...
Background ─────┬─ idle_generation_loop → リジェクション音声事前生成
                └─ idle_acceptance_generation_loop → 受諾音声事前生成
```
...
└── src/
    ...
    ├── speech_generator.py    # LLM統合・文章生成パイプライン
    ├── rejection_stock.py     # リジェクション音声事前生成 (max 100)
    └── acceptance_stock.py    # 受諾音声事前生成 (max 50)
```
...
```json
{
  ...
  "text_generated": "お願いがあります。給湯室でコーヒー豆の補充をお願いします。",
...
### `GET /api/voice/acceptance/random`
ストックからランダムな受諾音声を返却。

### `GET /api/voice/acceptance/status`
受諾音声ストックの状態を返却。

### `POST /api/voice/acceptance/clear`
ストックを全削除し再生成を開始。
...
### 二重音声 (Dual Voice)
タスク作成時に「アナウンス」と「完了メッセージ」を同時生成。完了メッセージはタスク内容に文脈的に関連した表現になります。
...
docker logs soms-voice | grep -i rejection
```
