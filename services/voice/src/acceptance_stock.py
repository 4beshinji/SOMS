"""
Acceptance voice stock manager.

Pre-generates acceptance/encouragement voice lines during idle time using LLM + VOICEVOX.
Maintains a stock of up to MAX_STOCK audio files for instant playback when users
accept tasks on the dashboard.
"""
import asyncio
import json
import uuid
...
MAX_STOCK = 50
STOCK_DIR = Path("/app/audio/acceptances")
...
class AcceptanceStock:
    """Manages pre-generated acceptance/encouragement voice audio stock."""
    ...
    def __init__(self, speech_gen: SpeechGenerator, voice_provider: TTSProvider):
        ...
        # Tracks number of active voice service requests (non-acceptance)
    ...
    def _init_storage(self):
        ...
        if MANIFEST_PATH.exists():
            try:
                ...
                self._save_manifest()
                logger.info(f"Acceptance stock loaded: {len(self._entries)} entries")
            except Exception as e:
                logger.warning(f"Failed to load acceptance manifest: {e}")
                self._entries = []
        else:
            self._entries = []
            self._save_manifest()
    ...
    async def get_random(self) -> dict | None:
        ...
        async with self._lock:
            ...
            try:
                ...
                return None
        logger.info(
            f"Served acceptance audio: {entry['text']} "
            f"(stock remaining: {self.count})"
        )
        return {
            "audio_url": f"/audio/acceptances/{entry['audio_file']}",
            "text": entry["text"],
        }
    ...
    async def clear_all(self):
        ...
        async with self._lock:
            ...
            self._entries = []
            self._save_manifest()
        logger.info("Acceptance stock cleared")

    async def generate_one(self) -> bool:
        """
        Generate one acceptance entry (LLM text + VOICEVOX audio).
        ...
        try:
            # 1. Generate acceptance/encouragement text via LLM
            text = await self.speech_gen.generate_acceptance_text()

            # 2. Synthesize audio (voice="acceptance" selects appropriate tone)
            result = await self.voice_provider.synthesize(text, voice="acceptance")

            # 3. Save audio file
            entry_id = str(uuid.uuid4())[:8]
            audio_filename = f"acceptance_{entry_id}.mp3"
            ...
            logger.info(
                f"Generated acceptance entry: '{text}' (stock: {self.count})"
            )
            return True

        except Exception as e:
            logger.error(f"Failed to generate acceptance entry: {e}")
            return False


async def idle_acceptance_generation_loop(stock: AcceptanceStock):
    """
    Background loop that generates acceptance audio during idle time.
    Runs continuously, sleeping between attempts.
    """
    logger.info("Acceptance stock idle generation loop started")
    ...
    while True:
        try:
            if stock.needs_refill and stock.is_idle:
                logger.debug(
                    f"Idle generation: acceptance stock={stock.count}, "
            ...
            logger.info("Acceptance idle generation loop cancelled")
            break
        except Exception as e:
            logger.error(f"Acceptance idle generation loop error: {e}")
            await asyncio.sleep(IDLE_INTERVAL)
