...
import type { ShoppingItem, PurchaseHistory } from '@soms/types';
import {
  fetchShopping,
  fetchShoppingStats,
...
const ShoppingPanel = memo(function ShoppingPanel() {
  ...
  const dueQuery = useQuery({
    queryKey: ['shopping-due'],
    queryFn: fetchShoppingDue,
  ...
  const historyQuery = useQuery({
    ...
    queryFn: () => fetchShoppingHistory(30),
    enabled: tab === 'history',
    staleTime: 30000,
  });
  ...
  const invalidate = useCallback(() => {
    ...
    queryClient.invalidateQueries({ queryKey: ['shopping-due'] });
  }, [queryClient]);
  ...
  const dueIds = new Set((dueQuery.data ?? []).map(d => d.id));
    ...
    <div className="bg-[var(--gray-800)] rounded-xl border border-[var(--gray-700)] overflow-hidden">
      ...
      <div className="px-4 py-3 border-b border-[var(--gray-700)] flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          {dueIds.size > 0 && (
            <span className="bg-red-500/80 text-white text-[10px] px-1.5 py-0.5 rounded-full font-medium animate-pulse">
              {dueIds.size} 期限
            </span>
          )}
        </div>
      ...
      {/* Tabs */}
      <div className="flex border-b border-[var(--gray-700)]">
        <button
          className={`flex-1 py-2 text-xs font-medium transition-colors ${tab === 'list' ? 'text-[var(--gray-100)] border-b-2 border-[var(--primary)]' : 'text-[var(--gray-500)] hover:text-[var(--gray-300)]'}`}
          onClick={() => setTab('list')}
        >
          リスト
        </button>
        <button
          className={`flex-1 py-2 text-xs font-medium transition-colors ${tab === 'history' ? 'text-[var(--gray-100)] border-b-2 border-[var(--primary)]' : 'text-[var(--gray-500)] hover:text-[var(--gray-300)]'}`}
      ...
      <div className="px-4 py-3 space-y-3">
        ...
        {tab === 'list' ? (
                ...
                <div key={cat}>
                  ...
                  {catItems.map(item => {
                    ...
                    return (
                      <div key={item.id}>
                        <div
                          className={`flex items-center gap-2 py-1 group ${isDue ? 'bg-red-900/20 -mx-1 px-1 rounded' : ''}`}
                        ...
                        >
                          <input
                            ...
                            checked={false}
                            onChange={(e) => { e.stopPropagation(); purchaseMutation.mutate(item.id); }}
                          ...
                          {isDue && (
                            <span className="text-[9px] text-red-400 bg-red-900/30 px-1 rounded font-medium">
                              期限
                            </span>
                          )}
                          {item.is_recurring && (
                        ...
                        {isExpanded && item.notes && (
                          <div className="ml-6 mb-1 text-[10px] text-[var(--gray-400)] bg-[var(--gray-700)] rounded p-1.5">
                            {item.notes}
                          </div>
                        )}
                      </div>
          ...
          historyQuery.isLoading ? (
            ...
            <p className="text-xs text-[var(--gray-500)] py-4 text-center">購入履歴はありません</p>
          ) : (
            <div className="space-y-1">
              {(historyQuery.data ?? []).map((h: PurchaseHistory) => (
                <div key={h.id} className="flex items-center gap-2 py-1 text-xs">
                  <span className="text-[var(--gray-200)] flex-1 truncate">{h.item_name}</span>
                  {h.category && (
                    <span className={`text-[9px] px-1 rounded ${CATEGORY_COLORS[h.category] || 'bg-[var(--gray-700)] text-[var(--gray-400)]'}`}>
                      {h.category}
                    </span>
                  )}
                  {h.store && (
                    <span className="text-[9px] text-[var(--gray-500)] bg-[var(--gray-700)] px-1 rounded">{h.store}</span>
                  )}
                  {h.price != null && (
                    <span className="text-[10px] text-[var(--gray-500)]">¥{h.price.toLocaleString()}</span>
                  )}
                  <span className="text-[10px] text-[var(--gray-500)]">
                    {new Date(h.purchased_at).toLocaleDateString('ja-JP', { month: 'numeric', day: 'numeric' })}
                  </span>
                </div>
              ))}
            </div>
