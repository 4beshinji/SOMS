...
class TestGetCurrentUser:
    ...
    def test_valid_token_returns_user(self):
        token = make_token(sub=42, username="alice", display_name="Alice A")
        client = TestClient(_create_app())
        resp = client.get("/optional", headers={"Authorization": f"Bearer {token}"})
        assert resp.status_code == 200
        data = resp.json()["user"]
    ...
    def test_expired_token_returns_none(self):
        token = make_token(exp_delta_sec=-60)
        client = TestClient(_create_app())
        resp = client.get("/optional", headers={"Authorization": f"Bearer {token}"})
        assert resp.status_code == 200
        assert resp.json()["user"] is None
    ...
    def test_just_expired_token_returns_none(self):
        ...
        token = make_token(exp_delta_sec=-1)
        client = TestClient(_create_app())
        resp = client.get("/optional", headers={"Authorization": f"Bearer {token}"})
        assert resp.status_code == 200
        assert resp.json()["user"] is None
    ...
    def test_wrong_secret_returns_none(self):
        token = make_token(secret="wrong_secret_key_that_is_long!!")
        client = TestClient(_create_app())
        resp = client.get("/optional", headers={"Authorization": f"Bearer {token}"})
        assert resp.status_code == 200
        assert resp.json()["user"] is None
    ...
    def test_large_user_id(self):
        token = make_token(sub=2147483647)  # max 32-bit int
        client = TestClient(_create_app())
        resp = client.get("/optional", headers={"Authorization": f"Bearer {token}"})
    ...
    def test_unicode_display_name(self):
        token = make_token(display_name="田中太郎")
        client = TestClient(_create_app())
        resp = client.get("/optional", headers={"Authorization": f"Bearer {token}"})
    ...
    def test_none_algorithm_rejected(self):
        ...
        payload_b64 = base64.urlsafe_b64encode(json_mod.dumps(payload_data).encode()).rstrip(b"=").decode()
...
class TestRequireAuth:
    ...
    def test_wrong_issuer_returns_401(self):
        token = make_token(iss="evil-service")
        client = TestClient(_create_app())
        resp = client.get("/required",
                          headers={"Authorization": f"Bearer {token}"})
        assert resp.status_code == 401
    ...
    def test_valid_token_returns_user(self):
        token = make_token(sub=7, username="bob", display_name="Bob B")
        client = TestClient(_create_app())
        resp = client.get("/required",
                          headers={"Authorization": f"Bearer {token}"})
        assert resp.status_code == 200
