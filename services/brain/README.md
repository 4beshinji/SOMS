...
- **最大反復**: 1サイクル 5イテレーション
- **ポート**: なし (外部 HTTP なし。Dashboard API / Voice API を REST クライアントとして呼び出す)
...
├── device_registry.py      デバイス登録・トラスト管理
├── wallet_bridge.py        MQTT ハートビート → Wallet XP 報酬変換
...
| ツール | 用途 | 主なパラメータ |
|--------|------|--------------|
| `create_task` | ダッシュボードにタスク作成 | title, description, bounty (500-5000), urgency (0-4), zone |
