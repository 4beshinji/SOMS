...
SOMS は Core Hub アーキテクチャの最初の実装。オフィス1部屋を対象に、GPU1台 + Docker 12サービスで全フローを動作検証する。クラウド月額費用 $0。

| 機能 | 技術 | 状態 |
|---|---|---|
| 自律判断 (脳) | Qwen3.5 14B (llama.cpp, ROCm), ReActループ (6ツール, 最大5反復, 3層安全機構) | 稼働 |
...
AMD RX 9700 (RDNA4) + Qwen3.5 14B (Q4_K_M, llama.cpp): LLM推論 ~51 tok/s 以上, 正常時応答 3.3秒, ツール呼び出し 6.6秒
...
GPU1台。Docker 12サービス。クラウド月額 $0。50,000:1 のデータ圧縮。
