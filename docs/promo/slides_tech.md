...
SOMS はオフィス1部屋を対象にした最初のCore Hub実装。GPU1台、Docker 12サービスで全機能が稼働する。

| 生体アナロジー | コンポーネント | 技術 | 実装状況 |
|---|---|---|---|
| 脳 | ReAct認知ループ | Qwen3.5 14B (llama.cpp, ROCm) | 6ツール, 最大5反復, 3層安全機構 |
...
| `get_device_status` | デバイスのネットワーク・バッテリー状態確認 | なし (読み取り専用) |
...
Backend: FastAPI + SQLAlchemy async。PostgreSQL 16 (asyncpg)。タスクモデル27カラム (bounty, urgency, voice関連, queue/assignment, federation, spatial)。
...
| コンポーネント | 状態 | 詳細 |
|---|---|---|
| ReAct認知ループ | 稼働 | 6ツール, 最大5反復, 3層安全機構 |
...
| E2E統合テスト | 稼働 | 7シナリオ |
| Event Store | 稼働 | 4ファイル, イベント永続化・クエリ |
| Data Mart | 未実装 | Phase 1 で対応 |

---

# パフォーマンス実測値

AMD RX 9700 (RDNA4) + Qwen3.5 14B (Q4_K_M, llama.cpp) — GPU サーバー1台構成
...
| 層 | 技術 | 補足 |
|---|---|---|
| LLM | Qwen3.5 14B (Q4_K_M GGUF), llama.cpp server (ROCm, continuous batching) | Mock LLM: キーワードマッチでtool call生成 |
...
| Frontend | React 19, TypeScript, Vite 7, Tailwind CSS 4 | TanStack Query 5, Framer Motion, Lucide icons |
...
| Database | PostgreSQL 16 (asyncpg) | 非同期ORM (SQLAlchemy async) |
| Container | Docker Compose (12 services) | ROCm対応 |
...
```
llama.cpp + Qwen3.5 14B (GGUF) + YOLOv11 + 実ESP32
...
| Wallet | 127.0.0.1:8003 | soms-wallet |
| Wallet PWA | 8004 | soms-wallet-app |
| PostgreSQL | 5432 | soms-postgres |
| VOICEVOX | 50021 | soms-voicevox |
| LLM (llama.cpp) | 11434 | soms-llm |
| MQTT | 1883 | soms-mqtt |
...
GPU1台のサーバーに12のDockerサービスを載せ、センサーデータの収集からLLM推論、タスク生成、音声通知、報酬支払いまでの全フローがローカルで完結する。外部へのデータ送信はゼロ。
