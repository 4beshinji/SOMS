...
|-------------------|-------------------|
| 脳 (Brain) | LLM (Qwen3.5 via llama.cpp / Mock LLM) |
...
| モデル | 主要フィールド | 用途 |
|-------|--------------|------|
| Task | title, description, urgency(0-4), skill_level, zone, assigned_to, accepted_at, announcement/completion_audio_url/text | タスク管理 |
| TaskAuditLog | task_id, action (created/accepted/dispatched/completed), actor_user_id, notes, region_id, timestamp | ライフサイクル監査 |
| VoiceEvent | message, audio_url, tone, zone | 音声イベント記録 (speak ツール用) |
| User | username, display_name, is_active | ユーザー |
| SystemStats | tasks_completed, tasks_created | シングルトン統計 |
...
### 7.4 Task Audit Log (コンプライアンス証跡)

v2 では v1 の Wallet/複式簿記/クレジット経済はすべて削除された (労働法・資金決済法の制約、`docs/architecture/v2-b2b-migration.md` 参照)。代わりに追加されたのが **`task_audit_log`** — 金額を含まない、タスクライフサイクルの追記専用証跡。

`services/dashboard/backend/` の Task ルーターが各イベントで 1 行追記する:

| action | 書込みタイミング | actor_user_id |
|--------|----------------|----------------|
| `created` | POST `/tasks/` 成功時 | none (system) |
| `accepted` | PUT `/tasks/{id}/accept` 成功時 | body.user_id |
| `dispatched` | PUT `/tasks/{id}/dispatch` 成功時 | none |
| `completed` | PUT `/tasks/{id}/complete` 成功時 | auth_user.id or assigned_to |

読取エンドポイント (require_auth):
- `GET /tasks/audit?limit=N` — 全タスク横断のフィード
- `GET /tasks/{task_id}/audit` — 単一タスクの履歴

Admin SPA の **Activity** タブがこのフィードを表示する。
...
**JWT仕様**: `{ sub: user_id, username, display_name, iss: "soms-auth" }` — `JWT_SECRET` を auth/dashboard 間で共有。
...
[T+4s]  LLM (Think): "CO2が高い。在室者がいるので換気が必要"
        LLM (Act): get_active_tasks() → 既存の換気タスクなし
                   create_task(title="キッチンの換気", urgency=3)

[T+5s]  Sanitizer: urgency=3 ∈ [0,4] ✓
        → DashboardClient: POST /tasks/ → タスク作成 + task_audit_log 書込み
...
| 層 | 技術 | 用途 |
|----|------|------|
| **LLM** | Qwen3.5 (9B/14B, GGUF Q4_K_M), llama.cpp server (ROCm) / Mock LLM | 推論エンジン |
...
│   │   ├── chat_server.py      #   aiohttp HTTP :8080 (chat / /devices/status / model mgmt)
...
│   ├── dashboard/
│   │   ├── backend/            #   FastAPI + SQLAlchemy + Sensor/Spatial/Device API + task_audit_log
│   │   │   └── repositories/  #   Repositoryパターン (Sensor + Spatial)
│   │   └── frontend/src/       #   React 19 + TanStack Query + AudioQueue
│   ├── admin/frontend/         # 管理者コンソール (React 19)
│   │   └── src/pages/          #   Analytics / TaskQueue / Activity (audit) / Shopping / Inventory / …
│   ├── voice/src/              # VOICEVOX 連携
│   │   ├── main.py             #   FastAPI エンドポイント
│   │   ├── speech_generator.py #   LLM テキスト生成 (announce / acceptance / rejection)
...
- **llama.cpp + Qwen3.5 (9B/14B GGUF)**: 本物の LLM 推論 (ROCm, ~51 tok/s 以上 — llama.cpp の方が Ollama より高速。51 tok/s は AMD RX 9700 上での旧 Ollama 計測値で、保守的な下限)
