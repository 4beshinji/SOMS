...
## 2. SOMSの回答: 自己増殖する分散知能

> **都市が自身のデータを、自身の計算資源で、自身の論理で処理する。**
> **そしてそのネットワークは、経済的インセンティブによって自律的に拡張する。**

SOMSは単なるセンサーネットワークではない。**自分から広がるシステム**である。

### 2.1 自己増殖サイクル

```
[1] CoreHub がセンサーデータを解釈
         │
[2] 物理タスクを検知し、人間に委託 (クレジット報酬付き)
         │
[3] タスク完了 → クレジット発行 + デバイスXP蓄積
         │
[4] XPが高いゾーン → 報酬乗数が上昇 (最大3.0x)
         │
[5] 高報酬 → より多くのセンサー設置・より多くの人間参加
         │
[6] センサー密度向上 → LLMの判断精度向上 → [1] に戻る
```

このフィードバックループにより、システムは外部からの管理なしに成長する。

### 2.2 インセンティブ構造

| 参加者 | 行動 | 報酬 | 結果 |
|--------|------|------|------|
| **センサー提供者** | ESP32を設置して電源を入れる | デバイスXP + 時間報酬 (500/時) | センサー密度が上がる |
| **GPU提供者** | CoreHubを運用する | インフラ報酬 (5,000/時) | 推論能力が増える |
| **タスク遂行者** | LLMが検知した物理タスクを実行 | クレジット (500-5,000) | 空間が改善される |
| **デバイス投資者** | 他人のセンサーにクレジットを出資 | 配当 | 資本がセンサー網に流れる |

**デマレッジ (保有税 2%/日)**: クレジットを保有するだけで日次2%減少。使うか投資するかの選択を促し、経済循環を強制する。
...
| Credit Ledger | 複式簿記、デバイスXP、デマレッジ、投資 | `services/wallet/` |
...
| 「どのゾーンにセンサーを追加すれば最もROIが高いか？」 | XP・報酬・タスクデータ |
...
- [x] クレジット経済 (複式簿記, XP, デマレッジ, デバイス投資, P2P送金)
- [x] Dashboard (React 19) + Voice (VOICEVOX) + Mobile Wallet (PWA)
- [x] OAuth認証 (Slack / GitHub + JWT)
- [x] SwitchBot連携 (9デバイス種)
...
CoreHubが自動認識し、デバイスXPが蓄積され、報酬が発生する。
...
| **City Data Hub** | 複数CoreHubの集約データを統合分析するサーバー |
...
| **デマレッジ** | 保有クレジットの日次減少 (2%)。経済循環を促進する保有税 |
| **デバイスXP** | センサー稼働で蓄積される経験値。報酬乗数 (最大3.0x) を決定 |
...
|------|------|
| `docs/SYSTEM_OVERVIEW.md` | 技術的全体像 |
| `docs/CURRENCY_SYSTEM.md` | クレジット経済の全貌 |
