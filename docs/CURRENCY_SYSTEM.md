...
| Dashboard Backend | `services/dashboard/backend/` | 8000 | PostgreSQL (asyncpg) |
...
### 3.2 Dashboard Backend (PostgreSQL)
...
> **2026-02-20 時点**: 本セクションに記載されていた既知の問題 (App.tsx マージコンフリクト、Task モデルのカラム欠損、WalletPanel 不整合、users.py スタブ) は **すべて解決済み**。
...
services/wallet/
├── src/
│   ├── main.py              FastAPIアプリ、起動時初期化 + system wallet作成
│   ├── database.py          asyncpg エンジン
│   ├── models.py            Wallet, LedgerEntry, Device, DeviceStake, FundingPool, PoolContribution, SupplyStats, RewardRate
│   ├── schemas.py           Pydantic 2.x スキーマ (15+種)
│   ├── services/
│   │   ├── ledger.py        複式簿記 transfer()
│   │   ├── xp_scorer.py     ゾーンXP付与、報酬乗数計算、貢献度重み付け
│   │   ├── stake_service.py デバイス出資: open/close/buy/return/distribute_reward
│   │   ├── pool_service.py  プール出資: create/contribute/activate
│   │   ├── demurrage.py     デマレッジ (保有税) サイクル
│   │   └── monetary_policy.py 金融政策パラメータ
│   └── routers/
│       ├── wallets.py        残高照会、ウォレット作成
│       ├── transactions.py   取引履歴、タスク報酬API、P2P送金、手数料プレビュー
│       ├── devices.py        デバイス登録/一覧/ハートビート/XP/utility-score
│       ├── stakes.py         Model A: SOMS出資 (6EP + portfolio)
│       ├── pools.py          Model B: プール出資 (admin 5EP + public 1EP)
│       └── admin.py          供給統計、デマレッジ、報酬レート

services/dashboard/backend/
├── models.py                         Task (bounty_gold, assigned_to, accepted_at)
├── schemas.py                        TaskCreate, TaskAccept, etc.
└── routers/tasks.py                  完了時の wallet 連携 (fire-and-forget)
