...
| Dashboard Frontend | 23 .tsx/.ts | ~1,450 | 完成 (TanStack Query、authFetch JWT統合、LLM Timelineチャート、ErrorBoundary) |
...
| `src/auth/authFetch.ts` | 59 | JWT認証付きfetchラッパー (401自動リフレッシュ+リトライ) |
| `src/hooks/useAnalytics.ts` | 168 | センサー/LLMデータ用TanStack Queryフック (authFetch統合) |
| `src/pages/AnalyticsPage.tsx` | 758 | Analytics: ゾーン概要、時系列チャート、LLM Timelineバーチャート、イベントフィード |
...
| 低 | 占有ヒートマップ可視化 | spatial_heatmap_hourly のフロントエンド表示 |
