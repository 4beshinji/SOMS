...
| Perception (画像認識) | 25 .py | ~3,050 | 完成 (MTMC人物追跡 + ArUcoキャリブレーション追加) |
| Dashboard Backend | 19 .py | 2,124 | 完成 (Sensor API + Spatial API + Device Position API + JWT認証) |
...
| ユニットテスト (7サービス) | 45 .py | ~5,500 | 587件 (Brain 167, Auth 97, Voice 79, Dashboard 71, Wallet 64, SwitchBot 59, Perception 50) |
| **合計** | **~280** | **~33,000** | |
...
71372a3 fix: enable HTTPS for wallet-app to allow camera access for QR scanning
bafc846 fix: add hard guard for alert suppression to prevent duplicate env tasks
...
| 中 | Dashboard Frontend JWT統合 | Auth Serviceは完成、フロントエンド認証フロー未実装 |
...
| Dashboard | 71 | `test_jwt_auth.py`, `test_auth_endpoints.py` | JWT認証ミドルウェア、認証保護エンドポイント (タスク受諾/完了) |
...
| Perception | 50 | `test_aruco_calibrator.py`, `test_reid_embedder.py`, `test_tracklet.py` | ArUcoキャリブレーション、ReID特徴量、トラックレット管理 |
...
.venv/bin/python -m pytest services/dashboard/backend/tests/  # Dashboard: 71件
.venv/bin/python -m pytest services/wallet/tests/         # Wallet: 64件
.venv/bin/python -m pytest services/switchbot/tests/      # SwitchBot: 59件
.venv/bin/python -m pytest services/perception/tests/     # Perception: 50件
```
...
| U | ドキュメント整備, テスト拡充 (587件: Brain 167 + Auth 97 + Voice 79 + Dashboard 71 + Wallet 64 + SwitchBot 59 + Perception 50) | — |
